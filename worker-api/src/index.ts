...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Route: GET /admin/jobs - Poll for pending encoding jobs
    ...
    // Route: PATCH /admin/jobs - Update job status
    if (url.pathname === '/admin/jobs' && request.method === 'PATCH') {
      try {
        ...
        if (status === 'completed') {
          const job = await env.DB.prepare("SELECT video_id FROM encoding_jobs WHERE id = ?").bind(jobId).first() as { video_id: string };
